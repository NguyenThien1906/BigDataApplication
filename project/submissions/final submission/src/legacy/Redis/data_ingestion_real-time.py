...
def read_users_rating(fname):
    print("Đang đọc rating dataset ...")
    chunk_size = 1000  # Đọc 100,000 dòng mỗi lần để tăng tốc
    data_ingestion_iter = 0
    for chunk in pd.read_csv(fname, chunksize=chunk_size):
        #    chunk = chunk.fillna("")  # Xử lý NaN
        data = chunk.to_dict(orient="records")
        data_ingestion_iter += 1
        print(f"Data ingest iter: {data_ingestion_iter}.")
        ...
        print("Data ingestion pause.")
        time.sleep(10)
        print("Data ingestion continued. ", end='')
...
if __name__ == "__main__":
    ...
    #fname_details = config_json['userdetails']
